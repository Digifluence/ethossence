/**
 * IMAGE RESOURCES GALLERY WITH LIGHTBOX
 * HANDLES MODAL DISPLAY, NAVIGATION, AND KEYBOARD CONTROLS
 * SUPPORTS AUTHORIZATION-FILTERED IMAGE RESOURCES
 */

class ImageResourceGallery extends HTMLElement {
  constructor() {
    super();
  ...
  init() {
    ...
    this.querySelector('.image-resource-gallery-modal__content')?.addEventListener('click', (e) => {
      e.stopPropagation();
    });
  }
  ...
  loadImageData() {
    ...
    try {
      ...
      console.error('ERROR PARSING IMAGE RESOURCE GALLERY DATA:', error);
      return [];
    }
  }
  ...
  openModal(index) {
    ...
    document.body.classList.add('image-resource-gallery-modal-open');
  ...
  closeModal() {
    ...
    document.body.classList.remove('image-resource-gallery-modal-open');
...
if (!customElements.get('image-resource-gallery')) {
  customElements.define('image-resource-gallery', ImageResourceGallery);
}
